...
| 3.2 | **FCM push notifications** | ✅ Done — `device_tokens` + `bookings.arrival_notified_at`; `FcmService` (legacy HTTP send, feature-gated `FEATURE_PUSH`); `NotificationService` routes any notification's `toFcm()` payload to FCM; `TripService::notifyArrivingPassengers()` fires `UserArrivedAtPickup` + arrival notification within `push.arrived_radius_m` (idempotent); `POST/DELETE /api/v1/push/tokens`; PWA service worker `push`/`notificationclick` deep-links to the ride (since v0.25.0) | "500m away" nudges need to reach a closed browser | `NotificationService` sends FCM; `UserArrivedAtPickup` fires it |
...
| 3.4 | **Ride-credit reminders** | ✅ Done — `SendRideCreditRemindersJob` (nightly, `time_bank.remind_within_days` default 3) + `RideCreditDueSoon` notification (database + log) + idempotent `reminder_sent_at` stamp; overdue/repaid/waived never reminded | Overdue book needs gentle churn-recovery | Pre-due SMS/database reminder job + test |
| 3.5 | **Employer enrollment CSV self-service** | ✅ Done — `EmployerEnrollmentController` with CSV upload, admin toggle, member removal; `EnsureEmployerAdmin` middleware; `employer/enrollment/index.blade.php` dashboard; 13 tests pass | MDAs should manage their own rosters | MDA-scoped upload page behind `employer_admin` |
| 3.6 | **Corridor fare config UI** | ✅ Done — `settings` table + `SettingsService` (override-first, config fallback) + `/admin/settings` Control Tower page (per-corridor naira inputs, blank restores default, change-control `corridor_fare_updated` trail); `PricingService::fareFor()` reads overrides | Ops must tune fares without deploys | `/admin/settings` writes `workride.*` to DB/cache |
| 3.7 | **Export `maatwebsite/excel`** | CSV exports exist everywhere; **Trust ledger + pay-it-forward now Excel** | FERMA + subsidy reports asked for real .xlsx | `composer require maatwebsite/excel`; admin exports xlsx |
| 3.8 | **EV lease-to-own schema seams** | ✅ Done (schema seams gated `FEATURE_EV_LEASE`) — `assets.propulsion`, `telemetry.battery_soc`/`range_km`, `lease_agreements` (fuel baseline, outstanding/progress), `charging_stations`; hardware/lease-owning DEFERed per design review | Only if EV pilot lands | `assets.propulsion`, `telemetry.battery_soc`, `lease_agreements`, `charging_stations` behind `FEATURE_EV_LEASE` |
| 3.9 | **Forecast Phase 2 ML job** | ✅ Done — `CalculateDemandForecastJob` (4-week same-weekday+hour baseline × event multiplier, 14-day horizon, nightly + manual train) + `demand_forecasts` table + `ForecastService::learned()` on the Demand Calendar | "Predicted = avg × multiplier" is manual; ML predicts automatically | `CalculateDemandForecastJob` trains on bookings history |
...
| 3.11 | **Pay-it-forward statement** | ✅ Done — `/admin/trust/pay-it-forward` (monthly: rode / repaid / overdue / waived with cash + seat volumes, per-rider breakdown) + CSV export + ledger entry | Board governance | Monthly report: who rode / repaid / overdue / waived |
...
| 3.14 | **Subsidy CSR/certificates for employers** | ✅ Done — `EmployerReportService` + `/admin/employers/{id}/report` printable monthly CO₂/fuel/trips/subsidy aggregate per employer | Employer renewals need one-click CO₂ reports | `/admin/employers/{id}/report` printable |
