...
| Feature modules | ✅ UI Rebrand Phase 0 complete — Apple × Elon world-class design system: `design-system.css` with semantic tokens (Forest #1B5E20, Gold #F9A825, Slate #0F172A, Paper #F8FAF8), Liquid-Glass glassmorphism, motion tokens, safe-area insets, high-contrast + reduced-motion support, 44×44 touch targets. Core mobile components: RouteChip (was CorridorChip), LiveTripCard, BottomSheet, PrimaryButton (56px min, haptic), StatusBadge, WalletPill (Office Support first), ImpactBadge, EmptyState (demand CTA). Layouts: `layouts/app` (auth) + `layouts/public` (guest-safe) with Dynamic Type support. Global bottom tab bar (Go, My Rides, Wallet, Impact, Profile) + FAB (Add My Vehicle). Dark mode toggle + system preference. Language files: `en.json` (130+ keys simple English), `ha.json`, `yo.json`, `ig.json`, `en_NG.json` — all using Nigerian simple English (no "corridor", "trip", "booking" — shows "Route", "Ride", "Join Ride"). |
| Feature modules | ✅ UI Rebrand Phase 1 complete — Go Board (`/go`) Screen 3: destination-first home with interactive Leaflet/OSM map (user location pulse, vehicle pins with seat badges, demand heat circles sized by `passenger_volume_daily` from 45 seeded junctions), sliding bottom sheet with drag handle, horizontal RouteChips (Kubwa→Central Area, Nyanya/Mararaba→Idu, Lugbe/Gwagwalada→Garki, Inside Town), window preset chips (Now/Later/Tomorrow/Any), LiveTripCards stacked (driver avatar+L3 badge, route title, departure+Leaving soon, live seats via Reverb, fare/Free, match score reasons, IRI color dot, primary Join Ride/Hold Seat), demand-aware empty state + Be the driver CTA, floating "Where are you going?" search pill with junction/workplace/OSM results. API: `GET /api/v1/go/board?window=` for pull-to-refresh. Components created: `RouteChip`, `LiveTripCard`, `BottomSheet`, `PrimaryButton`, `StatusBadge`, `WalletPill`, `ImpactBadge`, `EmptyState`. Services: `JourneyService` for live journey tracking (15s driver location updates, 100m waypoint geofence arrival, ETA calc, SOS/Share audit trail). |
| Tests | ✅ 581 feature tests passing (… + JunctionIntelTest (5): seeder writes intel columns, search seeded-volume fallback, survey totals win over seeded volume, junctionCounts intel keys, admin hub badge render) |
