...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen bg-paper pb-20 lg:pb-0">
        ...
        <header class="sticky top-0 z-40 border-b border-ink-200/70 bg-white/80 backdrop-blur-lg">
            <div class="mx-auto flex h-16 max-w-6xl items-center justify-between gap-4 px-4">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('wallet.index') }}" class="hidden items-center gap-2 rounded-full border border-ink-200 bg-white px-3 py-1.5 text-xs font-semibold text-ink-900 transition hover:border-forest-300 md:flex">
                    ...
                    <button type="button" @click="$dispatch('open-command')"
                            class="hidden items-center gap-1.5 rounded-lg border border-ink-200 bg-white px-2.5 py-1.5 text-xs font-medium text-ink-500 transition hover:border-forest-300 hover:text-ink-700 md:flex"
                            aria-label="Search (⌘K)">
