    ...
    >
        ...
        >
            ...
            <div class="absolute top-4 left-4 right-4 z-20" x-data="whereTo" @destination-selected.window="select($event.detail)">
                <div class="relative max-w-2xl mx-auto">
                    <button
                        ...
                        :class="[
                            'w-full h-14 rounded-xl bg-white/95 backdrop-blur-sm shadow-lg border border-ink-200 px-5 py-3 text-left transition-all',
                    ...
                    >
                        <div class="flex items-center gap-3">
                            ...
                            <span class="flex-1 font-semibold text-ink-900 truncate" x-text="selected ? selected.name : 'Where are you going?'"></span>
                            <x-icon name="search" class="h-5 w-5 text-ink-400 flex-shrink-0" />
                        </div>
                    </button>
                    ...
                    <div
                        ...
                        class="absolute left-0 right-0 z-30 mt-2 max-h-[60vh] overflow-auto rounded-xl border border-ink-200 bg-white shadow-xl"
                        role="listbox"
                    ...
                    >
                        <div class="p-2 space-y-1">
                            <template x-for="r in results" :key="r.name + r.type + (r.id ?? '')">
                                <button
                                    ...
                                    role="option"
                                    @click="select(r)"
                                    class="flex w-full items-center justify-between gap-3 rounded-lg px-3 py-3 text-left text-sm transition hover:bg-forest-50 focus:outline-none focus:ring-2 focus:ring-forest-500/30"
                                >
                                    <span class="min-w-0 flex items-center gap-3">
                                        <span
                                            :class="[
                                                'flex h-8 w-8 items-center justify-center rounded-full',
                                                r.type === 'junction' ? 'bg-forest-100 text-forest-700' : 'bg-gold-100 text-gold-700',
                                            ]"
                                            aria-hidden="true"
                                        >
                                        ...
                                        <span class="flex flex-col">
                                            ...
                                            <span class="block text-xs text-ink-500 flex items-center gap-1.5">
                            ...
                            <template x-if="results.length === 0 && !searching">
                                <div class="px-3 py-4 text-center text-sm text-ink-500">
                        ...
                        {{-- Quick actions --}}
                        <div class="border-t border-ink-100 p-2 space-y-1">
                            <a
                                href="{{ route('demand.index') }}"
                                class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm text-forest-700 hover:bg-forest-50 transition"
                            >
            ...
            <div class="absolute bottom-4 left-4 right-4 z-10 md:left-auto md:right-4 md:bottom-auto md:top-4 md:w-56">
                <div class="rounded-xl bg-white/95 backdrop-blur-sm shadow-lg border border-ink-200 p-3">
                    <p class="text-xs font-semibold text-ink-700 mb-2">Map Legend</p>
                    <div class="space-y-1.5 text-xs text-ink-600">
                        ...
                        <div class="flex items-center gap-2 pt-1 border-t border-ink-100">
            ...
            <button
                type="button"
                @click="locateUser()"
                class="absolute bottom-4 right-4 z-10 md:bottom-auto md:top-20 rounded-xl bg-white/95 backdrop-blur-sm shadow-lg border border-ink-200 p-2.5 transition hover:bg-forest-50 hover:border-forest-300 focus:outline-none focus:ring-2 focus:ring-forest-500/30"
                            ...
                            @foreach ($windowPresets as $key => $minutes)
                                <button
                                    ...
                                    class="inline-flex min-h-[40px] items-center rounded-full border px-3 py-1.5 text-sm font-medium transition
                                        {{ $key === 'now' ? 'bg-forest-600 text-white border-forest-600' : 'bg-white text-ink-700 border-ink-200 hover:border-forest-300' }}"
                                >
                                    {{ ucfirst($key) }}
                                </button>
                            @endforeach
