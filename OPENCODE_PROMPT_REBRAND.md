...
### PROMPT STATUS — LEARNINGS MERGED FROM `suggestion.txt` (2026-08-06)

This prompt now also absorbs the navigation "from-to" critique, the restrained-motion & branding principles, and the open-source package shortlist from `suggestion.txt`. Completed steps are marked ✅ and must NOT be re-done; re-run their tests only. Steps still open are marked ⬜.

| Step | Scope | Status |
|------|-------|--------|
| 3.1 | Design system tokens, glass, a11y | ✅ Done — `design-system.css` (tokens + motion scale + reduced-motion collapse) |
| 3.2 | Global shell & nav (bottom tabs, top nav, public layout) | ✅ Done — `layouts/app`, `layouts/public`, mobile tab bar, ⌘K palette, profile menu |
| 3.3 | Map-first board + corridor chip hero | ✅ Done — map-first board, corridor chips with live pulse + per-corridor count + min fare (`TripMatchingService::corridorStats`), live seat counter (`TripSeatsUpdated`), leaving-soon sort, demand-aware empty state |
| 3.4 | Booking flow + connect guide | ✅ Done — payment picker (`x-payment-picker`, large tappable rows, submit spinner) + 3-state connect guide (overview → active HUD → arrived/missed), from-to journey framing, optional voice announcements |
| 3.5 | My Rides + Wallet + Impact + Profile | ✅ Done — My Rides segmented Active/Upcoming/Past with Open Guide CTA; Wallet 3 balances; Impact certificates QR-verifiable; Profile & safety |
| 3.6 | Driver & publish | ✅ Done — publish + fleet gate banner + driver fleet app UI |
| 3.7 | Polish, dark mode, empty/loading/error | ⬜ Open — dark mode, loading skeletons, systematic error states |
| New A | From-to journey framing + voice in the connect guide (§5 of suggestion.txt) | ✅ Done this session — origin_text → boarding point strip + Walk chip; opt-in Web Speech distance nudges every ~100 m, arrived/missed spoken, off by default |
| New B | Open-source package shortlist (suggestion.txt §7) | ⬜ Adopt selectively per PART 5 below — do NOT bulk-install |
| New C | Restrained motion & branding principles (suggestion.txt tail) | ✅ Partially — motion tokens + one-shot pulses landed in v0.20.0; keep every new animation behind `WORKRIDE_ANIMATIONS` + `prefers-reduced-motion` |
...
#### STEP 3.3 - MAP-FIRST TRIPS BOARD (HIGHEST LEVERAGE) — ✅ DONE

> Implemented. `TripMatchingService::corridorStats(int $withinMinutes)` returns `['count' => int, 'min_fare' => ?int]` per corridor; `trips/board.blade.php` chips render `· N` and `· ₦min` (or `FREE` at 0) plus the live `wr-pulse` dot gated by `TripMatchingService::liveCorridors()`. The board is map-first (Leaflet, live trips pinned at coords, scheduled trips at corridor anchors), live seats push into the map, and the 48h/30min board-vs-matcher split is preserved. Do NOT rebuild — only extend.
...
#### STEP 3.4 - BOOKING FLOW + CONNECT GUIDE (THE AWARD WINNER - DO NOT SKIP) — ✅ DONE

> Implemented. Payment picker ships as `resources/views/components/payment-picker.blade.php` + `resources/js/payment-picker.js` (large 56px tappable rows, checkmark, single "Confirm seat · ₦X" button, submitting spinner, `active:scale-[0.98]` press feedback, free-ride label). It posts `payment_method` exactly as before — `wallet|cash|subsidy_credit|ride_credit` (for free rides it posts a real method like `wallet`; the service stamps `PaymentMethod::Free` internally — never post `free`, the controller rejects it). The connect guide ships as `/trips/{trip}/guide` with the three states (overview → active → arrived/missed), private-channel live vehicle target, walking route via `RoutingService` foot profile, 50 m arrived radius, and full a11y. See PART 5 New-A for the from-to framing + voice layer added on top.
...
#### STEP 3.5 - MY RIDES, WALLET, IMPACT — ✅ DONE

> Implemented. `BookingController::index()` splits bookings into `$activeBookings` / `$upcomingBookings` / `$pastBookings`; `bookings/index.blade.php` renders an Alpine segmented control (Active | Upcoming | Past with counts, empty-state panels, driver trips section below) and each card delegates to `bookings/_booking-card.blade.php` which shows an **Open guide** CTA (`route('trips.guide.show', $trip)`) for confirmed/boarded bookings on scheduled/active trips, plus Cancel, Receipt (`receipts.booking`), and a rating form for completed unrated rides. Do NOT rebuild — only extend.
...
### PART 5 - LEARNINGS MERGED FROM `suggestion.txt`

#### 5-A. Connect guide — from-to framing + optional voice (✅ IMPLEMENTED, keep the contract)

The suggestion's "robot-eye / 3D Google-style navigation" critique stands: full 3D pitched camera with adaptive zoom/rotation is **wrong for WorkRide right now** (Leaflet + OSM + Alpine must stay light, and the hard job is the last 200–500 m at informal junctions, not city-wide turn-by-turn). What we adopt instead is a **clearer-than-Google last-mile connect guide** built on from-to framing:

- **From-To strip** (implemented in `trips/guide.blade.php` overview card): a journey row `origin_text (Trip starts, blue dot)` → dashed connector → `boarding point label (Meet here, forest dot)` + a `Walk` chip. Passenger always knows where they are going and where they connect.
- **Start guide** hands over from overview (quiet straight-line estimate) to the active HUD — never a dramatic camera pitch; `flyTo`/pan only, and instant jumps under `prefers-reduced-motion`.
- **Active HUD** shows live distance/ETA with 150 ms gold `wr-number-tick`, vehicle pin pulses once per `TripLocationUpdated` (never a constant beat), re-route threshold 150 m via `RoutingService` foot profile.
- **Voice announcements (new, opt-in)**: a `x-guide-voice-toggle` button (speaker icon, `aria-pressed`, 44×44, only rendered when `'speechSynthesis' in window`) flips `voiceEnabled` in `connect-guide-ui.js`. `connect-guide.js` fires `onVoice(text)` for: "Following the route to the green dot." on start, a distance nudge every ~100 m ("X metres away — the vehicle is approaching."), "You have arrived. Wave to the driver.", and "The ride is gone. Find another ride." on the missed state. Voice is **off by default**, stays on-device (Web Speech API, no audio uploaded), and respects the mute state — never announced without an explicit tap.
- **Terminal states**: arrived ≤ 50 m → success panel; missed (trip cancelled/completed without boarding, or own booking cancelled) → calm slate panel with Find another ride / Open trip page / Call driver.

#### 5-B. Open-source package shortlist (⬜ adopt selectively, do NOT bulk-install)

Only pull what directly closes a remaining gap; keep the critical path < 50 kb JS and 3G-friendly:

- **Maps / guide / geo**: Leaflet (present) + `leaflet-routing-machine` / `lrm-osrm` + Leaflet a11y plugins + `@turf/turf` only if the guide needs client-side geo math; `brick/geo` (PHP) only if Haversine stops being enough. No Google Maps JS for core flows.
- **Exports & certificates**: `maatwebsite/excel` for FERMA / subsidy / OD-matrix / settlements `.xlsx`; PDF via `barryvdh/laravel-dompdf` or `spatie/laravel-pdf` for premium certificates; `simplesoftwareio/simple-qrcode` already present.
- **Admin acceleration**: **Filament** for the Control Tower (fleet assignment, demand calendar, business KPIs) — but only if we accept a Livewire surface on the Ops side; the Rider PWA stays Blade+Alpine.
- **PWA/head**: `laravel/head` (official) for clean meta / Open Graph / theme-color management.
- **GTFS**: MobilityData GTFS Validator (run as a process) before Google submission; `trafiklab/gtfs-php-sdk` optional hardening.
- **Explicitly avoid**: heavy SPA rewrites (Vue/React), Google Maps JS as primary engine, Livewire everywhere, unmaintained geo libs.

#### 5-C. Restrained motion & branding principles (✅ partially — keep enforcing)

Motion = feedback and orientation, never decoration:

- Calm, consistent visual language (Forest Green, glass, clear icons) is itself the trust differentiator vs Google Maps/Bolt.
- One-shot pulses only (`wr-pulse`, `wr-seat-tick`, `wr-number-tick`, `wr-pin-soft`/`wr-pin-move`); pins must NOT breathe infinitely while the user walks.
- Never convey essential information by motion alone; everything collapses to 0.01 ms under `prefers-reduced-motion`.
- New decorative SVGs stay behind `WORKRIDE_ANIMATIONS=false`; the animated brand cards are already gated.
- Every new animation reuses the tokens in `design-system.css` (motion scale + easing), never ad-hoc keyframes in Blade.
